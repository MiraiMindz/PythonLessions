...
def get_user_values() -> str:
    ...
    _chapter = get_cap(int(_cp))
    return _name, _chapter
...
def main(args=None) -> None:
    ...
    def mkfiles():
        ...
        if not mdfpath.exists():
            with open(mdfpath, 'w', encoding='UTF-8') as _:
                pass
        if not pyfpath.exists():
            resp = input(
                'Gostaria de criar um arquivo em Python para acompanhar com codigo? (s/n) '
            )
            if resp.lower() == 's':
